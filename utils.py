...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0 and gt.sum() > 0:
        ...
        # 计算 Precision 和 Sensitivity (Recall)
        ...
        # 计算 ASD (Average Surface Distance)
        try:
        ...
        mae = np.mean(np.abs(pred.astype(float) - gt.astype(float)))

        return dice, hd95, hd, iou, mae, accuracy, precision, sensitivity, asd
